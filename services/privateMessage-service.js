...
const privateMessageServices = {
  async getPrivateMessages (senderId, receiverId) {
    ...
    const privateMessages = await sequelize.query(queryPrivateMessages, {
  ...
  async getPrivateUsersList (targetUserId) {
    ...
    const privateUsersList = await sequelize.query(queryPrivateMessages, {
      type: QueryTypes.SELECT,
  ...
  async setMessagesRead (senderId, receiverId) {
    ...
    const [a, countUnreadPrivateMessages] = await sequelize.query(queryString, {
  ...
  async countUnreadPrivateMessages (receiverId, senderId = null) {
    ...
    const results = await sequelize.query(queryString.value, {
      type: QueryTypes.SELECT,
      replacements: {
        senderId,
        receiverId
